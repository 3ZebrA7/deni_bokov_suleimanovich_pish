...
#функция для подсчёта статистики по одному месяцу
def month_stats_one(datas, month_filter):
    month_list = []

    for line in datas:
        if line["month"] == month_filter:
            month_list.append(line)

    if not month_list:
        print("WARNING: Нет данных по месяцу", month_filter)
        return None

    return year_stats(month_list)

#функция для подсчёта статистики по всем месяцам сразу
def month_stats_all(datas):
    months = {}
    for line in datas:
        m = line['month']
    ...
    for m in sorted(months):
        stats = year_stats(months[m])

        print("===============Месяц", m, "=================")
        print("СРЕДНЯЯ:      ", stats['avg'])
        print("МАКСИМАЛЬНАЯ: ", stats['max'])
        print("МИНИМАЛЬНАЯ:  ", stats['min'])
        print()
#общая функция, которая решает считать один месяц или все
def month_stats(datas, month_filter):
    if month_filter is None:
        month_stats_all(datas)
    else:
        stats = month_stats_one(datas, month_filter)
        if stats is not None:
            print("================Месяц", month_filter, "=================")
            print("СРЕДНЯЯ:      ", stats['avg'])
            print("МАКСИМАЛЬНАЯ: ", stats['max'])
            print("МИНИМАЛЬНАЯ:  ", stats['min'])
            print()

month_stats(line_datas, month_filter)
